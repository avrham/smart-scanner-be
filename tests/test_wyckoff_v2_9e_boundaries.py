...
class TestProductionSurfacesUnmodified:
    ...
    def test_all_migrations_untouched_and_no_new_one(self):
        ...
        assert migrations[-1] == "025_discovery_reference_session.sql"
        ...
        assert [m for m in migrations if m.startswith("018_")] == ["018_durable_job_queue.sql"]
        assert [m for m in migrations if m.startswith("019_")] == ["019_catalyst_events.sql"]
        assert [m for m in migrations if m.startswith("020_")] == ["020_company_news.sql"]
        assert [m for m in migrations if m.startswith("021_")] == ["021_sec_material_events.sql"]
        assert [m for m in migrations if m.startswith("022_")] == ["022_external_signals.sql"]
        assert [m for m in migrations if m.startswith("023_")] == ["023_external_discovery.sql"]
        assert [m for m in migrations if m.startswith("024_")] == [
            "024_market_calendar_and_analyst.sql"]
        assert [m for m in migrations if m.startswith("025_")] == [
            "025_discovery_reference_session.sql"]
        assert not [m for m in migrations if m.startswith("026_")]
